    ...
    def create_interface(self):
        with gr.Blocks(
            ...
            with gr.Tab("Agent"):
                with gr.Row():
                    topic_bx = gr.Textbox(
                        label="Essay Topic", value="Khác biệt giữa Vue2 và Vue3"
                    )
                    gen_btn = gr.Button(
                        "Start Generate", scale=0, min_width=80, variant="primary"
                    )
                    cont_btn = gr.Button("Continue Step", scale=0, min_width=80)
                with gr.Row():
